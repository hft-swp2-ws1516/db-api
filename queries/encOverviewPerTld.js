(function () {
    ...
    var startQuery = function() {
        ...
        Scan.aggregate([
            ...
            // group by domain and enc, so we get a object for every au of a domain
            { $group: {
                _id: {
                    domain: "$domain",
                    enc: "$enc"
            ...
            for (var i = 0; i < result.length; i++) {
                var encOverview = new EncOverview();
                encOverview.month = monthStart.year() + '_' + (monthStart.month()+1);
                encOverview.enc = result[i].enc;
                encOverview.tld = result[i].tld;
                encOverview.count = result[i].count;
                encOverviews.push(encOverview);
            }

            async.eachLimit(encOverviews, 10, function(encOverview, callback){
                // get plain data, which we will insert into mongo
                var plainData = encOverview.toObject();
                delete plainData._id;

                // save the aggregated data to mongo
                var upsertQuery = {month: encOverview.month, tld: encOverview.tld, enc: encOverview.enc};
                EncOverview.findOneAndUpdate(upsertQuery, plainData, {upsert:true}, function(err, doc){
                    callback();
                });
            }, function(){
                if (err) { throw err; }
